...
public class AdobeLtvEnabling {
    ...
    public void enable() throws OperatorException, GeneralSecurityException, IOException, StreamParsingException, OCSPException {
    ...
    private void addLtvForChain(X509Certificate certificate, IOcspClient ocspClient, ICrlClient crlClient, PdfName key) throws GeneralSecurityException, IOException, StreamParsingException, OperatorCreationException, OCSPException {
    ...
    private void outputDss() throws IOException {
        ...
        for (PdfName vkey : validated.keySet()) {
            System.out.println("vkey: " + vkey.getValue());
            ...
            if (crl.size() > 0) {
                ...
                System.out.println("crl.size() > 0 " + crl.size());
            }
            if (cert.size() > 0) {
                cert.makeIndirect(pdfDocument);
                vri.put(PdfName.Cert, cert);
                System.out.println("cert.size() > 0");
            }
    ...
    private static PdfName getCrlHashKey(byte[] crlBytes) throws NoSuchAlgorithmException, IOException, CRLException, CertificateException {
    ...
    private static PdfName getOcspHashKey(byte[] basicResponseBytes) throws NoSuchAlgorithmException, IOException {
    ...
    private static PdfName getSignatureHashKey(PdfSignature sig) throws NoSuchAlgorithmException, IOException {
    ...
    private static X509Certificate getOcspSignerCertificate(byte[] basicResponseBytes) throws CertificateException, OCSPException, OperatorCreationException {
    ...
    private static X509Certificate getIssuerCertificate(X509Certificate certificate) throws IOException, StreamParsingException {
